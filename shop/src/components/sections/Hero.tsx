  ...
  return (
    <div className="relative h-[540px] w-full">
      ...
      >
        ...
        {/* base board */}
        <motion.rect
          x="40" y="40" width="440" height="460" rx="24"
        ...
        {[
          ...
          <motion.circle
            ...
            transition={{ delay: 1 + i * 0.08, duration: 0.5, ease: "backOut" }}
          />
        ))}
      </svg>
    </div>
  ...
  return (
    <section ref={containerRef} className="relative overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none" aria-hidden="true">
        <motion.div
          className="hero-gradient-orb hero-orb-anim absolute -top-24 -left-24 h-72 w-72 bg-red-200/60"
          animate={{ y: [0, -20, 0], x: [0, 15, 0] }}
          transition={{ repeat: Infinity, duration: 8, ease: "easeInOut" }}
        />
        <motion.div
          className="hero-gradient-orb hero-orb-anim-2 absolute top-32 right-0 h-80 w-80 bg-teal-200/50"
          animate={{ y: [0, 25, 0], x: [0, -20, 0] }}
          transition={{ repeat: Infinity, duration: 10, ease: "easeInOut" }}
        />
        <motion.div
          className="hero-gradient-orb hero-orb-anim-3 absolute bottom-0 left-1/3 h-64 w-64 bg-violet-200/50"
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            >
              <Link
                href={SHOP_URL}
                className="inline-flex items-center justify-center rounded-xl bg-slate-900 px-6 py-3 text-base font-semibold text-white shadow-sm hover:bg-slate-800 transition-all hover:shadow-lg hover:shadow-slate-900/20"
              >
              ...
              <Link
                href="#how-it-works"
                className="inline-flex items-center justify-center rounded-xl bg-white px-6 py-3 text-base font-semibold text-slate-900 ring-1 ring-slate-300 hover:bg-slate-50 transition-all hover:shadow-md"
              >
                How It Works
              </Link>
            </motion.div>
          ...
          >
            <motion.div
              ref={cardRef}
              className="rounded-3xl bg-white ring-1 ring-slate-200 shadow-xl overflow-hidden tilt-card"
              initial={{ scale: 0.96, opacity: 0, rotateX: 10, rotateY: -10 }}
              animate={isInView ? { scale: 1, opacity: 1, rotateX: 0, rotateY: 0 } : { scale: 0.96, opacity: 0, rotateX: 10, rotateY: -10 }}
            ...
            <motion.div
              className="absolute -bottom-6 -left-6 rounded-2xl bg-white p-4 shadow-xl ring-1 ring-slate-200 tilt-card"
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-green-50 animate-pulse-glow">
            ...
            <motion.div
              className="absolute -top-4 -right-4 rounded-2xl bg-white p-4 shadow-xl ring-1 ring-slate-200 tilt-card"
              initial={{ opacity: 0, x: 20, y: -20 }}
              animate={isInView ? { opacity: 1, x: 0, y: 0 } : { opacity: 0, x: 20, y: -20 }}
