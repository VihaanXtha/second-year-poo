...
// Reads `redirect` from the query string during static prerendering, which
// requires a Suspense boundary around the component that calls useSearchParams.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof Error && err.message === "PHONE_VERIFICATION_REQUIRED") {
        setError("Phone verification is required before you can sign in. Please check your email for the verification link, or sign in with Google.");
      } else {
    ...
    <div className="flex min-h-[70vh] flex-1 items-center justify-center bg-slate-50 px-4 py-16">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-slate-600">Sign in to your Circuit Bazaar account</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <button
            ...
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-50"
          >
